        ...
        <div class="overflow-x-auto bg-white shadow-md rounded-lg">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-sm font-medium text-gray-600 uppercase">Icon</th>
                ...
                @forelse($amenities as $index => $amenity)
                    <tr>
                        <td class="px-6 py-4 text-sm text-gray-800">{{ $amenities->firstItem() + $index }}</td>
                        <td class="px-6 py-4 text-sm text-gray-900 font-semibold">{{ $amenity->name }}</td>
                        <td class="px-6 py-4 text-sm text-gray-700">
                            @if($amenity->icon)
                                <i class="{{ $amenity->icon }}"></i> <span class="ml-2">{{ $amenity->icon }}</span>
                            @else
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-6 py-4 text-center text-sm text-gray-500">No amenities found.</td>
                    </tr>
                @endforelse
                </tbody>
            </table>
